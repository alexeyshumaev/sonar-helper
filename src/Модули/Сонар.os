...
Функция ПолучитьПроекты(АдресСервера, Токен) Экспорт
	...
	Пока Истина Цикл
		...
		Для Каждого ОписаниеПроекта Из Ответ.components Цикл
			...
			НовыйПроект.Вставить("Идентификатор", ОписаниеПроекта.project);
...
// ПолучитьЗамечанияПоФильтру
//	Получает массив идентификаторов замечаний по фильтру
// Параметры:
//   АдресСервера  - Строка - Адрес (хост) сервера SonarQube
//   Токен  - Строка - Токен пользователя, от имени которого выполняются запросы к API
//   Проекты - Массив - Идентификаторы проектов для анализа
//   Фильтр - Структура - Фильтры для отбора замечаний
//		* Статусы - Строка - Статусы замечаний
//		* Пользователи - Строка - Идентификаторы пользователей, на которых назначены замечания
//
//  Возвращаемое значение:
//   Массив - Идентификаторы замечаний
//
Функция ПолучитьЗамечанияПоФильтру(АдресСервера, Токен, Проекты, Фильтр) Экспорт
	
	Замечания = Новый Массив();
	URLШаблон = "issues/search?ps=500&componentKeys=%1%2";
	Если Не ПустаяСтрока(Фильтр.Статусы) Тогда
		URLШаблон = URLШаблон + "&statuses=" + Фильтр.Статусы;
	КонецЕсли;
	Если Не ПустаяСтрока(Фильтр.Пользователи) Тогда
		Если НРег(СокрЛП(Фильтр.Пользователи)) = "false" Тогда
			URLШаблон = URLШаблон + "&assigned=false";
		Иначе
			URLШаблон = URLШаблон + "&assignees=" + Фильтр.Пользователи;
		КонецЕсли;
	КонецЕсли;
	URLШаблон = URLШаблон + "&p=";
	...
	Для Каждого Проект Из Проекты Цикл
		...
		КлючиКомпонентов = ПолучитьКлючиКомпонентов(АдресСервера, Токен, Проект, Фильтр.Статусы, Фильтр.Пользователи);
		КоличествоКомпонентов = КлючиКомпонентов.Количество();
		Для Ит = 0 По КоличествоКомпонентов - 1 Цикл
			КлючКомпонента = КлючиКомпонентов[Ит];
			НомерСтраницы = 1;
			...
			Пока Истина Цикл
				...
				Если Ответ.total > МаксимальныйРазмерПорцииДанных Тогда
					// прочитаем компоненты, перенесем их в общий список, а по текущем прочтем только те, что висят именно на нем
					КлючиДочернихКомпонентов = ПолучитьКлючиКомпонентов(АдресСервера, Токен, КлючКомпонента, Фильтр.Статусы, Фильтр.Пользователи, Ложь);
					Для Каждого КлючДочернегоКомпонента Из КлючиДочернихКомпонентов Цикл
						КлючиКомпонентов.Добавить(КлючДочернегоКомпонента);
					КонецЦикла;
					КоличествоКомпонентов = КлючиКомпонентов.Количество();
				...
				Для Каждого ОписаниеОшибки Из Ответ.issues Цикл
					Замечания.Добавить(ОписаниеОшибки.key);
				КонецЦикла;
...
// ПолучитьСводнуюИнформациюПоЗамечаниямиПроекта
//	Возвращает свернутую таблицу с пользователями и их замечаниями на проекте проекта 
// Параметры:
//  АдресСервера  - Строка - Адрес (хост) сервера SonarQube
//  Токен  - Строка - Токен пользователя, от имени которого выполняются запросы к API
//  ОписаниеПроекта  - Структура - Описание проекта SonarQube
...
//		* Код - Строка - Код (Ключ) проекта
//  СтатусыСтрокой - Строка - Строка с идентификаторами статусов замечаний
//  Замечания - ТаблицаЗначений - Таблица со сводной информацией замечаний, аналогичная выходной. Используется при необходимости дополнять для нескольких проектов
//
// Возвращаемое значение:
//   ТаблицаЗначения   - Содвная таблица с замечаниями
//		* КлючПроекта - Строка - Ключ проекта sonarqube
//		* Статус - Строка - Статус замечания
//		* Автор - Строка - Пользователь, автор замечания по git (email)
//		* Отвественный - Строка, NULL - Пользователь, на которого назначено замечание (учетная запись)
//		* Правило - Строка - Ключ правила
//		* Критичность - Строка - Критичность замечания
//		* Тип - Строка - Тип замечания
//		* Время - Число - Время на исправление замечания (в минутах)
//		* Количество - Число - Количество замечаний
//
Функция ПолучитьСводнуюИнформациюПоЗамечаниямиПроекта(АдресСервера, Токен, ОписаниеПроекта, СтатусыСтрокой, Замечания = Неопределено) Экспорт
	...
	Если Замечания = Неопределено Тогда
		Замечания = Новый ТаблицаЗначений();
		Замечания.Колонки.Добавить("КлючПроекта");
		Замечания.Колонки.Добавить("Статус");
		Замечания.Колонки.Добавить("Автор");
		Замечания.Колонки.Добавить("Отвественный");
		Замечания.Колонки.Добавить("Правило");
		Замечания.Колонки.Добавить("Критичность");
		Замечания.Колонки.Добавить("Тип");
		Замечания.Колонки.Добавить("Время");
		Замечания.Колонки.Добавить("Количество");
	КонецЕсли;
	...
	Для Ит = 0 По КоличествоКомпонентов - 1 Цикл
		...
		Пока Истина Цикл
			...
			Для Каждого ОписаниеОшибки Из Ответ.issues Цикл
				...
				Замечание = Замечания.Добавить();
				Замечание.КлючПроекта = ОписаниеПроекта.Код;
				Замечание.Автор = ОписаниеОшибки.author;
				Если Не ОписаниеОшибки.Свойство("assignee") Тогда
					Замечание.Отвественный = NULL;
				Иначе
					Замечание.Отвественный = ОписаниеОшибки.assignee;
				КонецЕсли;
				
				Замечание.Правило = ОписаниеОшибки.rule;
				Замечание.Статус = ОписаниеОшибки.status;
				Замечание.Критичность = ОписаниеОшибки.severity;
				Замечание.Тип = ОписаниеОшибки.type;
				Замечание.Количество = 1;
				Замечание.Время = 0;
				
				Если ОписаниеОшибки.Свойство("debt") Тогда
					ВремяСтрокой = СтрЗаменить(ОписаниеОшибки.debt, "min", "");
					Поз = СтрНайти(ВремяСтрокой, "h");
					Если Поз <> СтрДлина(ВремяСтрокой) Тогда
						ВремяСтрокой = СтрЗаменить(ВремяСтрокой, "h", "*60+");
					Иначе
						ВремяСтрокой = СтрЗаменить(ВремяСтрокой, "h", "*60");
					КонецЕсли;

					Поз = СтрНайти(ВремяСтрокой, "d");
					Если Поз <> СтрДлина(ВремяСтрокой) Тогда
						ВремяСтрокой = СтрЗаменить(ВремяСтрокой, "d", "*480+"); // рабочие дни
					Иначе
						ВремяСтрокой = СтрЗаменить(ВремяСтрокой, "d", "*480");
					КонецЕсли;

					Замечание.Время = Вычислить(ВремяСтрокой);
	...
	Замечания.Свернуть("КлючПроекта, Автор, Отвественный, Правило, Критичность, Тип, Статус", "Время, Количество");
...
// НазначитьЗамечания
//	Выполняет назначение замечаний на указанного пользователя
// Параметры:
//  АдресСервера  - Строка - Адрес (хост) сервера SonarQube
//  Токен  - Строка - Токен пользователя, от имени которого выполняются запросы к API
//  Замечания - Массив - Ключи замечаний для переназначения
//  НазначитьНа - Строка - Идентификатор пользователя, на которого необходимо назначить замечание
//
Процедура НазначитьЗамечания(АдресСервера, Токен, Замечания, НазначитьНа) Экспорт
	
	URL = СтрШаблон("issues/bulk_change?assign=%1&issues=", НазначитьНа);

	ЗамечанияДляОбработки = Новый Массив();
	Порция = 500;
	Для Каждого Замечание Из Замечания Цикл
		
		Если ЗамечанияДляОбработки.Количество() = Порция Тогда
			КлючиЗамечаний = СтрСоединить(ЗамечанияДляОбработки, ",");
			ВыполнитьЗапрос(АдресСервера, Токен, URL + КлючиЗамечаний, "POST");
			ЗамечанияДляОбработки.Очистить();
		КонецЕсли;
...
Функция ПолучитьКоличествоЗамечанийКомпонента(АдресСервера, Токен, КлючКомпонента, СтатусыСтрокой = "", НазначеныНа = "")
	
	URL = "issues/search?ps=1&componentKeys=" + КлючКомпонента;
	Если Не ПустаяСтрока(СтатусыСтрокой) Тогда
		URL = URL + "&statuses=" + СтатусыСтрокой;
	КонецЕсли;
	Если Не ПустаяСтрока(НазначеныНа) Тогда
		Если НРег(СокрЛП(НазначеныНа)) = "false" Тогда
			URL = URL + "&assigned=false";
		Иначе
			URL = URL + "&assignees=" + НазначеныНа;
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьКлючиКомпонентов(АдресСервера, Токен, КлючКомпонента, СтатусыСтрокой, НазначеныНа = "", ТолькоКаталоги = Истина)
	
	КлючиКомпонент = Новый СписокЗначений();
	КоличествоЗамечанийПроекта = ПолучитьКоличествоЗамечанийКомпонента(АдресСервера, Токен, КлючКомпонента, СтатусыСтрокой, НазначеныНа);
